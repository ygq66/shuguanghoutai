  ...
  SetMap = (build, floor) => {
    ...
    floor.forEach((item2, index) => {
      this.SetMapFloor(
        item2.buildname,
        item2.floorname,
        item2.buildname + "#" + item2.floorname,
        "",
        floor.length - 1 === index
      )
    })
  }
  // 建筑请求
              ...
              {bulidFloor.map(menuObj => {
                return (
                  <li key={menuObj.id} className="addAlert">
                    <h2
                      className="alerth2"
                      onClick={(e) => this.onMenuClicked(e, menuObj, true)}
                      title={menuObj.build_name}
                    >
                      <img src={require("../../../assets/images/wenjianjia.png").default} alt=""/>
                      {menuObj.build_name}
                    </h2>
                    <ul>
